...
dat = dat %>% mutate(sanction = ifelse(year >= "2018-08-01", 1, 0))

dat = dat %>% mutate(sanction2 = ifelse(year >= "2018-09-01", 1, 0))

dat = dat %>% mutate(china = ifelse(Country == "China", 1, 0))

dat$did = dat$china*dat$sanction

dat$did2 = dat$china*dat$sanction2
...
ggplot(frame1, aes(year)) +
  geom_line(aes(y = rate_sk, linetype = "twodash")) +
  geom_line(aes(y = rate_ca, linetype = "dotted")) +
  geom_line(aes(y = rate_vt)) +
  ylim(-1, 1)
```

```{r}
dat_model = dat %>% filter(year >= "2012-01-01") %>% filter(Country == c("Vietnam", "South Korea", "China"))
did_model = lm(visitors ~ year + sanction + china + did, data = dat_model)
summary(did_model)
```
...
did_model = lm(value ~ year + sanction + china + did, data = dat_mod2)
summary(did_model)
```
