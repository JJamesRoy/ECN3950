...
# Peaufinage du dataset pour la 2e régression
...
#Création d'un tableau pour la 1ere régression
```
...
dat_es = dummy_cols(dat_e, select_columns = c("year","lead","lag"), remove_first_dummy = TRUE, remove_selected_columns = TRUE) # Créé les dummy pour les n-1 lag, lead et les effets fixes pour les mois
```
...
model_es = lm(value ~ . -sanction2 -did2 -country -did -sanction, data = dat_es)
summary(model_es)
...
did_model_alt = lm(value ~ sanction2 + china + did2, data = dat_model)
summary(did_model_alt)
```
...
dat_model2 = dat_model2 %>% select(-china)

did_model = lm(value ~ . -year -did2 -sanction2 -country, data = dat_model2)
summary(did_model)

did_model_alt = lm(value ~ . -year -did -sanction -country, data = dat_model2)
summary(did_model_alt)
```
